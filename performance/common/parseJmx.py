...
class JMeter:
    def __init__(self):
        ...
        self.raw_name = 'C:\\Users\\lee_j\\Desktop\\test.jmx'

    def read_jmeter_from_file(self):
        tree = ET.parse(self.raw_name)
        root = tree.getroot()
        test_plans = self.get_test_plan(root)
        return test_plans

    def get_test_plan(self, parent_node):
        test_plans_list = []
        try:
            ...
            for test_plan_hash_tree in test_plan_hash_trees:
                ...
                for test_plan in test_plans:
                    ...
                    test_plan_dict.update({'thread_group': self.get_threah_group(test_plan_hash_tree)})
    ...
    def get_threah_group(self, parent_node):
        thread_groups_list = []
        try:
            ...
            for thread_group_hash_tree in thread_group_hash_trees:
                ...
                for thread_group in thread_groups:
                    ...
                    thread_group_dict.update({'controller': self.get_controller(thread_group_hash_tree)})
    ...
    def get_controller(self, parent_node):
        controllers_list = []
        try:
            ...
            for controller_hash_tree in controller_hash_trees:
                ...
                for controller in controller_hash_tree:
                    ...
                    if controller.tag == 'hashTree' and controller.findall('HTTPSamplerProxy') and flag == 1:
                        flag += 1
                        controller_dict.update({'http_sample': self.get_http_sample(controller)})
    ...
    def get_http_sample(self, http_sample):
        https_list = []
        try:
            ...
            for i in range(len(http_sample_proxys)):
                ...
                http_dict.update({'arguments': self.get_arguments(http_sample_proxys[i])})
                http_dict.update({'sample_dict': self.get_http_sample_dict(http_sample_proxys[i])})
                http_dict.update({'assertion': self.get_assertion(http_sample_hash_trees[i])})
                http_dict.update({'extractor': self.get_extractor(http_sample_hash_trees[i])})
    ...
    def get_arguments(self, parent_node):
    ...
    def get_assertion(self, parent_node):
        """get assert"""
        try:
    ...
    def get_extractor(self, parent_node):
